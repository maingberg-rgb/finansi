            ...
            {isEditMode && (
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-3xl border-2 border-amber-200 p-6 mb-8 shadow-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-white rounded-2xl p-4">
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {(categories || [])
                                    ...
                                    .map(cat => (
                                        <div key={cat.id} className="flex items-center justify-between p-2 hover:bg-stone-50 rounded-lg group">
                                            <span className="font-bold text-stone-700">{cat.name}</span>
                                            <button
                                                onClick={() => {
                                                    if (window.confirm(`למחוק את הקטגוריה "${cat.name}"?\n\nשים לב: כל התנועות וההוצאות הקבועות המשויכות לקטגוריה זו יימחקו!`)) {
                        ...
                        {/* Income Categories */}
                        <div className="bg-white rounded-2xl p-4">
                            <h3 className="text-sm font-black text-emerald-700 mb-3">🟢 קטגוריות הכנסה</h3>
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {(categories || [])
                                    ...
                                    .map(cat => (
                                        <div key={cat.id} className="flex items-center justify-between p-2 hover:bg-stone-50 rounded-lg group">
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (window.confirm(`למחוק את הקטגוריה "${cat.name}"?\n\nשים לב: כל התנועות המשויכות לקטגוריה זו יימחקו!`)) {
                                                        deleteCategoryMutation.mutate(cat.id);
                                                    }
                                                }}
                                                className="p-1.5 text-stone-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                ...
                <div className="lg:col-span-2 bg-white rounded-3xl shadow-sm border border-stone-100 p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-bold text-stone-600 mb-2">קטגוריה</label>
                                <select
                                    value={categoryId}
